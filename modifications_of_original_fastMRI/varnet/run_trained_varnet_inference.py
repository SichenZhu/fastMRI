...
def run_inference(challenge, state_dict_file, data_path, output_path, device):
    ...
    # ------------- modification start -------------------
        ...
        state_dict = torch.load(state_dict_file, map_location=torch.device('cpu'))["state_dict"]
        state_dict_v2 = copy.deepcopy(state_dict)
        for key in state_dict:
            if 'varnet.' in key:
                nk = key.lstrip('varnet.')
                state_dict_v2[nk] = state_dict_v2.pop(key)
        del state_dict_v2['loss.w']
